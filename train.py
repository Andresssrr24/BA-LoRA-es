...
@dataclass
class BA_LoRA(Trainer):
    def __init__(self, *args, pretrained_model=None, lambda_cr=0.00001, lambda_dr=0.1, lambda_svdr=0.1, svd_k=10, **kwargs):
        super().__init__(*args, **kwargs)
        self.pretrained_model = pretrained_model
        self.lambda_cr = lambda_cr
        self.lambda_dr = lambda_dr
        self.lambda_svdr = lambda_svdr
        self.svd_k = svd_k
        self.global_step = 0
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        with torch.no_grad():
            pretrained_outputs = self.pretrained_model(**inputs)
            pretrained_logits = pretrained_outputs.logits
        reg_losses = compute_regularization_losses(
            pretrained_logits=pretrained_logits,
            fine_tuned_logits=logits,
            logits=logits,
            lambda_cr=self.lambda_cr,
            lambda_dr=self.lambda_dr,
            lambda_svdr=self.lambda_svdr,
            svd_k=self.svd_k,
        )
        total_loss = (task_loss + reg_losses["total_regularization_loss"]) 
